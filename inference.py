...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        for i in range(self.numParticles):
            self.particles.append(temp[i % leng])
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            trueDistances = [util.manhattanDistance(pos, pacmanPosition) for pos in p]
            temp = 1.0
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    temp *= emissionModels[i][trueDistances[i]]
            distributions[p] += temp
        
        if distributions.totalCount() == 0:
            self.initializeParticles()
        else:
            self.particles = [util.sample(distributions) for i in range(self.numParticles)]
