...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        if not self.Qtable.has_key((state, action)): self.Qtable[(state, action)] = 0.0
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            ...
            elif max_Q == self.getQValue(state, action):
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        maxQ = 0.0
        if len(self.getLegalActions(nextState)) != 0: maxQ = max(self.getQValue(nextState, a) for a in self.getLegalActions(nextState))
        self.Qtable[(state, action)] = self.getQValue(state, action) + self.alpha * (reward + self.discount * maxQ - self.getQValue(state, action))
        # util.raiseNotDefined()
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        maxQ = 0.0
        A_sp = self.getLegalActions(nextState)
        if len(A_sp) != 0: maxQ = max(self.getQValue(nextState, act) for act in A_sp)
