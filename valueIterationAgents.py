...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                if len(mdp.getPossibleActions(state)) == 0:
                    ...
                    tempValues[state] = max(self.getQValue(state, act) for act in mdp.getPossibleActions(state))
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, prob in P:
            _sum += (prob * (self.mdp.getReward(state, action, nextState) + self.discount * self.getValue(nextState)))
        return _sum
    ...
    def computeActionFromValues(self, state):
        ...
        for act in actions:
            v = 0
            for s, p in self.mdp.getTransitionStatesAndProbs(state, act):
                v += p * self.getValue(s)
            if v > value:
                action = act
                value  = v
